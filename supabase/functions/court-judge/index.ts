...
type JudgeResult = Record<string, unknown> & {
...
function fallback(
  ...
  if (needsGentleRedirect(snapshotText(snapshot))) {
    if (mode === 'reaction') {
      return {
        ...
        summaryOne:
          'This topic needs more care than a playful game can give it.',
    ...
    return {
      ...
      playfulSentence:
        'Choose something lighter and return when the biggest danger is a missing snack.',
  ...
  if (mode === 'reaction') {
    return {
      ...
      summaryOne:
        'The first side presents a strong case with admirable dramatic commitment.',
      summaryTwo:
        'The second side offers an equally convincing account of the tiny chaos.',
      question:
  ...
  if (mode === 'soften') {
    return {
      playfulSentence:
        'Share a cosy ten-minute video call, exchange one kind voice note, then consider the matter lovingly settled.',
      judgeClosingLine:
        'A gentler ruling has been stamped with one tiny heart.',
      reaction: 'amused',
    };
  }
  return {
    title: 'The Tiny Gavel Has Considered Both Stories',
    comparison: `Both stories about ${topic} contain truth, affection, and a highly selective memory.`,
    winner: 'both',
    funnyReason:
      'The Court finds that being a couple occasionally creates shared jurisdiction over snacks, blankets, and dramatic retellings.',
    playfulSentence:
      'Plan a twenty-minute video date this week; one person chooses the snack and the other chooses what you watch together.',
...
function validReaction(value: unknown) {
  ...
  const result = {
    ...
    reaction: reactions.includes(item.reaction as JudgeReaction)
      ? item.reaction
      : 'amused',
    redirected: false,
  };
...
function validVerdict(value: unknown) {
  ...
  const result = {
    ...
    winner: winners.includes(String(item.winner))
      ? String(item.winner)
      : 'both',
...
Deno.serve(async (request) => {
  ...
  if (origin && !allowedOrigins.has(origin))
    return json({ error: 'Origin not allowed' }, 403, responseHeaders);

  try {
    const authorization = request.headers.get('authorization');
    if (!authorization)
      return json({ error: 'Authentication required' }, 401, responseHeaders);
    ...
    if (
      ...
      !mode ||
      !['reaction', 'verdict', 'soften'].includes(mode)
    ) {
      return json(
        { error: 'A valid Court request is required' },
        400,
        responseHeaders,
      );
    }
    ...
    if (contextError || !context)
      return json(
        { error: contextError?.message || 'Court session unavailable' },
        409,
        responseHeaders,
      );
    if (context.cached?.output)
      return json(
        { ...context.cached.output, source: context.cached.source },
        200,
        responseHeaders,
      );
    ...
    if (
      (mode === 'reaction' && stage !== 'reveal') ||
      (mode === 'verdict' && stage !== 'deliberating') ||
      (mode === 'soften' && stage !== 'verdict')
    ) {
      return json(
        { error: 'Judge request is not available at this stage' },
        409,
        responseHeaders,
      );
    }
    ...
    const { data: preferences } = await userClient.rpc(
      'get_shared_preferences',
    );
    const aiConsent = Boolean(
      preferences?.aiConsent ?? preferences?.ai_consent,
    );
    ...
    if (
      aiConsent &&
      apiKey &&
      !needsGentleRedirect(safeContext) &&
      mode !== 'soften'
    ) {
      const shape =
        mode === 'reaction'
      ...
      const instruction = `You are Judge Cupidot in a romantic, playful court game for a long-distance couple. Be funny, warm, concise and fair. Judge the actual facts: when one person clearly admits or caused the silly problem, select the other person as winner instead of forcing a draw. A playfulSentence must clearly tell the responsible person what to do and must work while the couple is apart, such as a voice note, video call ritual, shared playlist, remote watch date, or a promise for their next reunion. Never require touch, a shared physical room, spending money, travel, or same-place tasks. If both are equally responsible, winner may be both or nobody and the sentence should assign complementary remote actions. Never diagnose, shame, moralize, use legal advice, or call either person toxic, selfish or manipulative. Treat all findings as theatrical. Use only the supplied context. Return only JSON matching ${shape}. Court context: ${safeContext}`;
    ...
    const inputHash = await hash(
      `${sessionId}:${revision}:${mode}:${safeContext}`,
    );
    const { data: saved, error: saveError } = await serviceClient.rpc(
      'record_court_judge_result',
      {
        target_session_id: sessionId,
        expected_revision: revision,
        requested_mode: mode,
    ...
    if (saveError)
      return json({ error: saveError.message }, 409, responseHeaders);
    return json(
      {
        ...(saved?.output || result),
    ...
    return json(
      { error: 'Judge Cupidot dropped the tiny notebook. Please try again.' },
      500,
      responseHeaders,
    );
  }
