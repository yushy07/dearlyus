...
describe('Cupidot companion moments', () => {
  it.each(['playful', 'tender', 'quiet'] as const)(
    'exhausts %s prompts before repeating and avoids immediate repeats',
    (mood) => {
      const seen: string[] = [];
      const count = COMPANION_MOMENTS.filter(
        (moment) => moment.mood === mood,
      ).length;
