...
export function AiConsentToggle() {
  ...
  return (
    <label style={{ display: 'flex', alignItems: 'flex-start', gap: '9px', color: 'var(--ink-soft)', fontSize: '12px', lineHeight: 1.45, cursor: 'pointer' }}>
      <input type="checkbox" checked={hasAiConsent} disabled={!isLoaded} onChange={(event) => { void setAiConsent(event.target.checked); }} style={{ marginTop: '3px', accentColor: 'var(--pink)' }} />
