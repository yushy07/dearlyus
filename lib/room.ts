...
export function useRoomSync({
  ...
  const sendEvent = useCallback(async (type: string, payload: unknown) => {
    ...
    if (!supabase || !roomId.current || !sessionId.current || !userId.current)
      return null;
        ...
        .on(
          ...
          (change: any) => {
            ...
            if (
              sessionId.current &&
              row.session_id &&
              row.session_id !== sessionId.current
            )
              return;
            receive({
              id: row.id,
