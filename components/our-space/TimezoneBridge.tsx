...
export function TimezoneBridge({
  ...
  const handleSaveScheduled = async () => {
    ...
    sounds.playChime();
    try {
      await saveScheduledDate(coupleId, new Date(scheduleTime).toISOString(), scheduleTitle);
  ...
  return (
    <div
      style={{
        background: 'linear-gradient(135deg, rgba(255,255,255,0.95), var(--paper-subtle, #FAF8F5))',
        borderRadius: '24px',
        padding: '24px',
    ...
    >
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
            <span style={{ fontSize: '12.5px', fontWeight: 700, color: 'var(--pink)' }}>
              🌸 {partnerA} ({cityA})
            </span>
          ...
          <div style={{ fontSize: '30px', fontWeight: 900, color: 'var(--ink)', letterSpacing: '-0.02em', lineHeight: 1.1 }}>
            {timeA.timeStr}
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '8px', marginBottom: '14px' }}>
        <div style={{ background: 'var(--paper-raised, #FFFFFF)', padding: '10px 12px', borderRadius: '12px', border: '1px solid var(--line)' }}>
          <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--ink-soft)' }}>⏱️ 15 MIN QUICK SYNC</div>
          <div style={{ fontSize: '12.5px', fontWeight: 700, color: 'var(--ink)', marginTop: '2px' }}>Bedtime Question</div>
        </div>

        <div style={{ background: 'var(--paper-raised, #FFFFFF)', padding: '10px 12px', borderRadius: '12px', border: '1px solid var(--line)' }}>
          <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--ink-soft)' }}>⏱️ 30 MIN SWEET SPOT</div>
          <div style={{ fontSize: '12.5px', fontWeight: 700, color: 'var(--ink)', marginTop: '2px' }}>Quiz Lore Sync</div>
        </div>

        <div style={{ background: 'var(--paper-raised, #FFFFFF)', padding: '10px 12px', borderRadius: '12px', border: '1px solid var(--line)' }}>
          <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--ink-soft)' }}>⏱️ 60 MIN FULL DATE</div>
          <div style={{ fontSize: '12.5px', fontWeight: 700, color: 'var(--ink)', marginTop: '2px' }}>Draw &amp; Ambient Radios</div>
        </div>
      </div>
        ...
        >
          <div>
            ...
            <div style={{ fontSize: '13.5px', fontWeight: 700, color: 'var(--ink)', marginTop: '2px' }}>
              {scheduledDate.title} · {new Date(scheduledDate.scheduledAt).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}
            </div>
          </div>
      ...
      {/* Scheduling Popover */}
      {schedulingOpen && (
        <div style={{ marginTop: '16px', padding: '16px', background: '#FFFFFF', borderRadius: '16px', border: '1px solid #FBCFE8' }}>
          <h4 style={{ fontSize: '14px', fontWeight: 700, margin: '0 0 10px', color: '#881337' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
            <button type="button" onClick={() => setSchedulingOpen(false)} className="btn btn-outline" style={{ padding: '6px 12px', fontSize: '12px' }}>
              Cancel
            </button>
            <button
              type="button"
